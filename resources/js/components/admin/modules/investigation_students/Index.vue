<template>
    <div class="position-absolute container-fluid" v-show="keys.includes( `investigation_student.list` )">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-6 text-start animate__animated animate__fadeInDown">
                        <h6>
                            <i class="lni lni-map-marker text-success me-2"></i>Investigaciones de Pregrado
                        </h6>
                    </div>
                    <div class="col-6 text-end mb-4">
                        ...
                        <button class="btn btn-sm btn-success mb-0" v-show="keys.includes( `investigation_student.store` )" @click="$refs.StoreComponent.store()">
                ...
                <div class="row">
                    <div id="filters" class="collapse">
                        <div class="col-md-12 col-sm-12 mb-4 text-start">
                            <find-component
                            id="investigation_studentFilt"
                            @filts="e => { filters = e, listing() }"
                            />
                        </div>
                    </div>
                </div>
                <div class="row position-relative" id="investigation_studentArea">
                    <table-component
                    :data="list.data"
        ...
        <store-component
        id="StoreInvestigationStudent"
        ref="StoreComponent"
        @reload="listing()"
        />
        <update-component
        id="UpdateInvestigationStudent"
        ref="UpdateComponent"
        @row="e => replace( list.data, e )"
        />
        <destroy-component
        id="DestroyInvestigationStudent"
...
export default
{
    ...
    mounted()
    {
        this.area = document.getElementById( `investigation_studentArea` )
    },
    ...
    methods:
    {
        listing()
        {
            ...
            this.axios.post( `${this.$uri}/investigation_student/list?page=${this.page}`, this.filters )
            .then( response =>
            {
