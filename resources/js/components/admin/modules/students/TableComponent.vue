    ...
    <div class="col-12">
        <div class="table-size-1">
            <table class="table table-hover align-items-center mb-0">
                <thead class="bg-th">
                    <tr>
                        <th class="col-3 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `person_id` )">
                            Estudiante
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                        <th class="col-6 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `code` )">
                            Codigo
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                        <th class="col-6 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `asignament_id` )">
                            Sede
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                ...
                <tbody>
                    <tr v-if="data?.length > 0" v-for="( element, index ) in data" :key="index">
                        <td class="align-middle text-left animate__animated animate__fadeIn">
                            <h6 class="mb-0 ms-2 text-xs">{{ element.program  }}</h6>
                        </td>
                        <td class="align-middle text-left animate__animated animate__fadeIn">
                            <h6 class="mb-0 ms-2 text-xs">{{ element.name  }}</h6>
                        </td>
                        <td class="align-middle text-center animate__animated animate__fadeIn">
                            <small class="mb-0 text-xs">{{ element.acronym  }}</small>
                        </td>
                        <td class="align-middle text-center animate__animated animate__fadeIn">
                        ...
                        <td class="align-middle text-center" v-show="keys.includes( `student.update` ) || keys.includes( `student.destroy` )">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropMenu">
                                <li class="cursor-pointer">
                                    <a class="dropdown-item" v-show="keys.includes( `student.update` )" @click="$emit( `update`, element )">
                                        <i class="lni lni-pencil-alt me-2"></i>Modificar
                                    </a>
                                </li>
                                <li class="cursor-pointer">
                                    <a class="dropdown-item" v-show="keys.includes( `student.destroy` )" @click="$emit( `destroy`, element )">
