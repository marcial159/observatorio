    ...
    <div class="col-12">
        <div class="table-size-1">
            <table class="table table-hover align-items-center mb-0">
                <thead class="bg-th">
                    <tr>
                        <th class="col-3 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `program` )">
                            Programa
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                        <th class="col-6 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `name` )">
                            Nombre
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                        <th class="col-6 text-uppercase text-xxs font-weight-bolder text-center" role="button" @click="$emit( `order`, `acronym` )">
                            Acronimo
                            <i class="lni lni-arrows-vertical"></i>
                        </th>
                ...
                <tbody>
                    <tr v-if="data?.length > 0" v-for="( element, index ) in data" :key="index">
                        ...
                        <td class="align-middle text-center" v-show="keys.includes( `career.update` ) || keys.includes( `career.destroy` )">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropMenu">
                                <li class="cursor-pointer">
                                    <a class="dropdown-item" v-show="keys.includes( `career.update` )" @click="$emit( `update`, element )">
                                        <i class="lni lni-pencil-alt me-2"></i>Modificar
                                    </a>
                                </li>
                                <li class="cursor-pointer">
                                    <a class="dropdown-item" v-show="keys.includes( `career.destroy` )" @click="$emit( `destroy`, element )">
